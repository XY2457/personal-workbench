...
import type { Expense, FixedExpense, ShoppingItem } from '../types'
...
export default function WealthWorkshop() {
  const [tab, setTab] = useState<'account' | 'analysis' | 'fixed' | 'shopping' | 'gold'>('account')
  ...
  const [accounts, setAccounts] = useState({ fixed: 0, income: 0, wish: 0 })
  ...
  const load = async () => {
    ...
    setAccounts({ fixed: monthFixed, income: monthIncome, wish: 0 })
  ...
  useEffect(() => { if (tab === 'gold') refreshGold() }, [tab])
  ...
  const deleteExpense = async (id: string) => { await dbDelete('expenses', id); load() }
  ...
  const deleteFixed = async (id: string) => { await dbDelete('fixed_expenses', id); load() }
  ...
  const deleteShop = async (id: string) => { await dbDelete('shopping_items', id); load() }
      ...
      {tab === 'analysis' && (
        <div>
          <div className="grid grid-2 mb-3">
            <div className="card"><div className="card-title">周支出趋势</div>
              <LineChart data={weekExpenses.map(d => d.value)} labels={weekExpenses.map(d => d.label)} color="#5A7A4A" />
            </div>
            <div className="card"><div className="card-title">月度支出结构</div>
              <PieChart data={categoryData.length > 0 ? categoryData : [{ label: '暂无', value: 1, color: '#ccc' }]} />
            </div>
          </div>
          <div className="grid grid-2">
            <div className="card"><div className="card-title">月收入结构</div>
              {incomeData.length > 0 ? <PieChart data={incomeData} /> : <div className="empty-state-text">暂无收入记录</div>}
            </div>
            <div className="card"><div className="card-title">本月收支结余</div>
        ...
        <div>
          ...
          <div className="grid grid-2">
            <div className="card">
              ...
              <div className="stat-number mb-2" style={{ fontSize: 18 }}>小计: ¥{unpurchasedItems.reduce((s, i) => s + i.price * i.quantity, 0).toFixed(2)}</div>
              {unpurchasedItems.map(item => (
                <div key={item.id} className="flex items-center justify-between" style={{ padding: '6px 0', borderBottom: '1px solid var(--color-border)' }}>
                  <div><span>{item.name}</span><span className="text-sm text-light"> ×{item.quantity}</span></div>
            ...
            <div className="card">
              ...
              {purchasedItems.map(item => (
                <div key={item.id} className="flex items-center justify-between" style={{ padding: '6px 0', borderBottom: '1px solid var(--color-border)', opacity: 0.7 }}>
                  <div><span style={{ textDecoration: 'line-through' }}>{item.name}</span><span className="text-sm text-light"> ×{item.quantity}</span></div>
                  <div className="flex items-center gap-1">
        ...
        <div>
          <div className="card mb-3">
            <div className="flex items-center justify-between">
              ...
              <button className="btn btn-sm btn-outline" onClick={refreshGold} disabled={goldLoading}>{goldLoading ? '刷新中...' : '↓ 下拉刷新'}</button>
            </div>
          </div>
          {goldPrice && (
            <>
              ...
              <div className="grid grid-2 mb-3">
                <div className="card"><div className="stat-number" style={{ fontSize: 18, color: 'var(--color-danger)' }}>¥{goldPrice.high.toFixed(2)}</div><div className="stat-label">7日最高</div></div>
                <div className="card"><div className="stat-number" style={{ fontSize: 18, color: 'var(--color-success)' }}>¥{goldPrice.low.toFixed(2)}</div><div className="stat-label">7日最低</div></div>
              </div>
              <div className="card mb-3"><div className="card-title">7日走势</div><LineChart data={goldPrice.history7d} color="#D4A03A" labels={Array.from({ length: 7 }, (_, i) => `${i + 1}天前`).reverse()} /></div>
              <div className="card mb-3"><div className="card-title">30日走势</div><LineChart data={goldPrice.history30d} color="#D4A03A" height={100} /></div>
              <div className="card">
